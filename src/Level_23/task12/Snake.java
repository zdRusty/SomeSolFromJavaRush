...
public class Snake {
     ...
     * Напрвление перемещения задано переменной direction.
     */
    public void move() {
        if (!isAlive) return;
    ...
    /**
     * Метод перемещает змею на соседнюю клетку.
    ...
    private void move(int dx, int dy) {
        ...
        if (head.getX() == mouse.getX() && head.getY() == mouse.getY()) // §г§м§Ц§Э§С
        {
